 ...
 */
vec3 EvaluateSpotLight(Buffer globalConst, Buffer lights, Payload payload, float normalBias, float viewBias, accelerationStructureEXT bvh) {
    vec3 color = vec3(0.f);
    for (uint lightIndex = 0; lightIndex < GetNumSpotLights(globalConst); lightIndex++) {
        // Get the index of the light
        uint index = (HasDirectionalLight(globalConst) + lightIndex);

        // Load the spot light
        Light spotLight = GetLight(lights, index);
 ...
 */
vec3 EvaluatePointLight(Buffer globalConst, Buffer lights, Payload payload, float normalBias, float viewBias, accelerationStructureEXT bvh) {
    vec3 color = vec3(0.f);
    for (uint lightIndex = 0; lightIndex < GetNumPointLights(globalConst); lightIndex++) {
        // Get the index of the point light
        uint index = HasDirectionalLight(globalConst) + GetNumSpotLights(globalConst);

        // Load the point light
        Light pointLight = GetLight(lights, index);
 ...
 */
vec3 EvaluateDirectionalLight(Buffer lights, Payload payload, float normalBias, float viewBias, accelerationStructureEXT bvh) {
    // Load the directional light data (directional light is always the first light)
    Light directionalLight = GetLight(lights, 0);
 ...
 */
vec3 DirectDiffuseLighting(Buffer globalConst, Buffer lights, Payload payload, float normalBias, float viewBias, accelerationStructureEXT bvh) {
    vec3 brdf = (payload.albedo / PI);
    vec3 lighting = vec3(0.f);

    if (bool(HasDirectionalLight(globalConst))) {
        lighting += EvaluateDirectionalLight(lights, payload, normalBias, viewBias, bvh);
    }

    if (GetNumSpotLights(globalConst) > 0) {
        lighting += EvaluateSpotLight(globalConst, lights, payload, normalBias, viewBias, bvh);
    }

    if (GetNumPointLights(globalConst) > 0) {
        lighting += EvaluatePointLight(globalConst, lights, payload, normalBias, viewBias, bvh);
    }
