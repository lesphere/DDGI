...
#include "include/ProbeCommon.glsl"

struct DDGIVolumeResources {
    Texture2DArray probeIrradiance;
    Texture2DArray probeDistance;
    Texture2DArray probeData;
...
vec3 DDGIGetVolumeIrradiance(vec3 worldPosition, vec3 surfaceBias, vec3 direction, DDGIVolumeDescGPU volume, DDGIVolumeResources resources) {
    ...
    for(int probeIndex = 0; probeIndex < 8; probeIndex++) {
        ...
        vec3 adjacentProbeWorldPosition = DDGIGetProbeWorldPosition(adjacentProbeCoords, volume, resources.probeData);
        ...
        vec3 trilinear = max(0.001f, mix(1.f - alpha, alpha, adjacentProbeOffset));
        ...
        vec2 filteredDistance = 2.f * TextureLod(resources.bilinearSampler, resources.probeDistance, probeTextureUV, 0).rg;
        ...
        vec3 probeIrradiance = TextureLod(resources.bilinearSampler, resources.probeIrradiance, probeTextureUV, 0).rgb;

        // Decode the tone curve, but leave a gamma = 2 curve to approximate sRGB blending
        vec3 exponent = volume.probeIrradianceEncodingGamma * 0.5f;
